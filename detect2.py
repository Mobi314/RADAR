...
def convert_pdf_to_image(pdf_path, user_choice, page_number):
    ...
    page = doc.load_page(page_number)  # Load specified page
...
def save_to_excel(table_data, workbook, sheet_name):
    """Save the table data to an Excel workbook, ensuring no unintended header row and clean data."""
    cleaned_data = []
    ...
    print(f"Adding data to sheet {sheet_name} in Excel workbook.")
    df.to_excel(workbook, sheet_name=sheet_name, index=False, header=False)
...
def select_pdf_and_convert():
    ...
    def on_export():
        ...
        if page_text == "all":
            ...
            pages = parse_page_selection(page_text)
        ...
        workbook = pd.ExcelWriter(f"output_{datetime.now().strftime('%Y-%m-%d_%H-%M-%S')}.xlsx", engine='openpyxl')

        for page_number in pages:
            image = convert_pdf_to_image(pdf_path, user_choice, page_number)
            if image is not None:
                detected_cells, _ = process_image_for_table_detection(image)
                if detected_cells:
                    table_data = extract_table_data(image, detected_cells)
                    save_to_excel(table_data, workbook, f"Page {page_number+1}")
        workbook.save()
...
def parse_page_selection(page_text):
    pages = set()
    parts = page_text.split(',')
    for part in parts:
        if '-' in part:
            start, end = map(int, part.split('-'))
            pages.update(range(start-1, end))  # Adjust for zero indexing
        else:
            pages.add(int(part) - 1)  # Adjust for zero indexing
    return list(pages)
